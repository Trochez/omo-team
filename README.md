...
### 🛡️ Master Supervision Protocol

**NEW in v1.1.0**: OMO-Team includes a master supervision protocol that minimizes token usage while preventing hallucinations and wrong-path execution.

- **Token-Efficient Supervision**: Master uses compact state files (`ledger.json`, `status.json`) and structured result headers (first 15 lines) for normal supervision, only escalating to full outputs on anomaly.
- **Evidence-Based Contracts**: Workers must include evidence paths, file modifications, and structured headers in their results.
- **Anomaly Detection**: Master flags missing/empty/malformed results, evidence mismatches, overlapping file claims, and out-of-scope modifications.
- **Failure Handling**: Preserves worker outputs, logs failures, and respawns with adjusted instructions (no master re-execution).
- **Role-Specific Evidence**: Each worker role (executor, searcher, researcher, consultant, reviewer, visual) must provide appropriate evidence anchors.
...
├── ledger.json            # Master-owned roll-up index
└── workers/
    ├── worker-1/
    │   ├── result.md      # Worker 1 output (with structured header)
    │   └── status.json    # Worker 1 compact status
    ├── worker-2/
